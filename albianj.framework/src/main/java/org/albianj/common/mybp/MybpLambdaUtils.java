 ...
 * @since 2018-05-10
 */
public final class MybpLambdaUtils {
...
//    private static final Map<String, Map<String, ColumnCache>> COLUMN_CACHE_MAP = new ConcurrentHashMap<>();
     ...
     * @return 返回解析后的结果
     */
    public static <T> MybpLambdaMeta extract(MybpSFunction<T, ?> func) {
        MybpLambdaMeta meta = null;
        // 1. IDEA 调试模式下 lambda 表达式是一个代理
        if (func instanceof Proxy) {
            meta = new MybpIdeaProxyMybpLambdaMeta((Proxy) func);
            return meta;
        }
        // 2. 反射读取
        try {
            ...
            meta = new MybpReflectLambdaMeta((SerializedLambda) method.invoke(func), func.getClass().getClassLoader());
        } catch (Throwable e) {
            // 3. 反射失败使用序列化的方式读取
            meta = new MybpShadowLambdaMeta(MybpLambdaSerializer.extract(func));
        }
        return meta;
    }
