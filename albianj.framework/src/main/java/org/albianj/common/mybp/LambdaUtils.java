...
//    private static final Map<String, Map<String, ColumnCache>> COLUMN_CACHE_MAP = new ConcurrentHashMap<>();
    ...
    public static <T> LambdaMeta extract(SFunction<T, ?> func) {
        LambdaMeta meta = null;
        // 1. IDEA 调试模式下 lambda 表达式是一个代理
        if (func instanceof Proxy) {
            meta = new IdeaProxyLambdaMeta((Proxy) func);
            return meta;
        }
        // 2. 反射读取
        try {
            ...
            meta = new ReflectLambdaMeta((SerializedLambda) method.invoke(func), func.getClass().getClassLoader());
        } catch (Throwable e) {
            // 3. 反射失败使用序列化的方式读取
            meta = new ShadowLambdaMeta(LambdaSerializer.extract(func));
        }
        return meta;
    }
