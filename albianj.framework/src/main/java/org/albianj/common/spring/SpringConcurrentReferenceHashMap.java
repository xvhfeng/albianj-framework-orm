 ...
 */
public class SpringConcurrentReferenceHashMap<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V> {
	 ...
	 */
	public SpringConcurrentReferenceHashMap() {
	 ...
	 */
	public SpringConcurrentReferenceHashMap(int initialCapacity) {
	 ...
	 */
	public SpringConcurrentReferenceHashMap(int initialCapacity, float loadFactor) {
	 ...
	 * write to the map
	 */
	public SpringConcurrentReferenceHashMap(int initialCapacity, int concurrencyLevel) {
	 ...
	 */
	public SpringConcurrentReferenceHashMap(int initialCapacity, ReferenceType referenceType) {
	 ...
	 * write to the map
	 */
	public SpringConcurrentReferenceHashMap(int initialCapacity, float loadFactor, int concurrencyLevel) {
	...
	public SpringConcurrentReferenceHashMap(
		...
		SpringAssert.isTrue(initialCapacity >= 0, "Initial capacity must not be negative");
		SpringAssert.isTrue(loadFactor > 0f, "Load factor must be positive");
		SpringAssert.isTrue(concurrencyLevel > 0, "Concurrency level must be positive");
		SpringAssert.notNull(referenceType, "Reference type must not be null");
	...
	public boolean containsKey( Object key) {
		Reference<K, V> ref = getReference(key, Restructure.WHEN_NECESSARY);
		Entry<K, V> entry = (ref != null ? ref.get() : null);
		return (entry != null && SpringObjectUtils.nullSafeEquals(entry.getKey(), key));
	}
	...
	private V put( final K key,  final V value, final boolean overwriteExisting) {
		return doTask(key, new Task<V>(TaskOption.RESTRUCTURE_BEFORE, TaskOption.RESIZE) {
			...
			protected V execute( Reference<K, V> ref,  Entry<K, V> entry,  Entries<V> entries) {
				...
				SpringAssert.state(entries != null, "No entries segment");
				entries.add(value);
				return null;
			}
		});
	...
	public boolean remove( Object key,  final Object value) {
		Boolean result = doTask(key, new Task<Boolean>(TaskOption.RESTRUCTURE_AFTER, TaskOption.SKIP_IF_EMPTY) {
			...
			protected Boolean execute( Reference<K, V> ref,  Entry<K, V> entry) {
				if (entry != null && SpringObjectUtils.nullSafeEquals(entry.getValue(), value)) {
					if (ref != null) {
						ref.release();
					}
					return true;
	...
	public boolean replace( K key,  final V oldValue,  final V newValue) {
		Boolean result = doTask(key, new Task<Boolean>(TaskOption.RESTRUCTURE_BEFORE, TaskOption.SKIP_IF_EMPTY) {
			@Override
			protected Boolean execute( Reference<K, V> ref,  Entry<K, V> entry) {
				if (entry != null && SpringObjectUtils.nullSafeEquals(entry.getValue(), oldValue)) {
			...
			while (currRef != null) {
				if (currRef.getHash() == hash) {
					...
					if (entry != null) {
						...
						if (SpringObjectUtils.nullSafeEquals(entryKey, key)) {
							return currRef;
						}
					}
				}
	...
	protected static final class Entry<K, V> implements Map.Entry<K, V> {
		...
		public boolean equals( Object other) {
			return (this == other || (other instanceof Map.Entry<?, ?> that &&
					SpringObjectUtils.nullSafeEquals(getKey(), that.getKey()) &&
					SpringObjectUtils.nullSafeEquals(getValue(), that.getValue())));
		}

		@Override
		public int hashCode() {
			return (SpringObjectUtils.nullSafeHashCode(this.key) ^ SpringObjectUtils.nullSafeHashCode(this.value));
		}
	...
	private class EntrySet extends AbstractSet<Map.Entry<K, V>> {
		...
		public boolean contains( Object o) {
			if (o instanceof Map.Entry<?, ?> entry) {
				Reference<K, V> ref = SpringConcurrentReferenceHashMap.this.getReference(entry.getKey(), Restructure.NEVER);
				Entry<K, V> otherEntry = (ref != null ? ref.get() : null);
				if (otherEntry != null) {
					return SpringObjectUtils.nullSafeEquals(entry.getValue(), otherEntry.getValue());
				}
			}
			return false;
		}
		...
		public boolean remove(Object o) {
			if (o instanceof Map.Entry<?, ?> entry) {
				return SpringConcurrentReferenceHashMap.this.remove(entry.getKey(), entry.getValue());
			}
			return false;
		}
		...
		@Override
		public int size() {
			return SpringConcurrentReferenceHashMap.this.size();
		}

		@Override
		public void clear() {
			SpringConcurrentReferenceHashMap.this.clear();
		}
	}
	...
	private class EntryIterator implements Iterator<Map.Entry<K, V>> {
		...
		private void moveToNextSegment() {
			...
			if (this.segmentIndex < SpringConcurrentReferenceHashMap.this.segments.length) {
				this.references = SpringConcurrentReferenceHashMap.this.segments[this.segmentIndex].references;
				this.segmentIndex++;
			}
		}
		...
		@Override
		public void remove() {
			SpringAssert.state(this.last != null, "No element to remove");
			SpringConcurrentReferenceHashMap.this.remove(this.last.getKey());
			this.last = null;
		}
	}
	...
	protected class ReferenceManager {
		...
		public Reference<K, V> createReference(Entry<K, V> entry, int hash,  Reference<K, V> next) {
			if (SpringConcurrentReferenceHashMap.this.referenceType == ReferenceType.WEAK) {
