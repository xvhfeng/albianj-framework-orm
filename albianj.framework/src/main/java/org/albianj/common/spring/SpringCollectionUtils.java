 ...
 * @since 1.1.3
 */
public abstract class SpringCollectionUtils {
	 ...
	 * aligned with Spring's own  and
	 * {@link SpringLinkedSpringMultiValueMap} constructor semantics as of 5.3.
	 * @param expectedSize the expected number of elements (with a corresponding
	 * capacity to be derived so that no resize/rehash operations are needed)
	 * @since 5.3
	 * @see #newHashMap(int)
	 ...
	 * @see SpringObjectUtils#toObjectArray(Object)
	...
	public static <E> void mergeArrayIntoCollection( Object array, Collection<E> collection) {
		Object[] arr = SpringObjectUtils.toObjectArray(array);
	...
	public static boolean contains( Iterator<?> iterator, Object element) {
		if (iterator != null) {
			while (iterator.hasNext()) {
				...
				if (SpringObjectUtils.nullSafeEquals(candidate, element)) {
					return true;
				}
			}
		}
	...
	public static Object findValueOfType(Collection<?> collection, Class<?>[] types) {
		if (isEmpty(collection) || SpringObjectUtils.isEmpty(types)) {
			return null;
		}
	 ...
	 * @since 3.1
	 */
	public static <K, V> SpringMultiValueMap<K, V> toMultiValueMap(Map<K, List<V>> targetMap) {
		return new SpringMultiValueMapAdapter<>(targetMap);
	}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public static <K, V> SpringMultiValueMap<K, V> unmodifiableMultiValueMap(
			SpringMultiValueMap<? extends K, ? extends V> targetMap) {

		SpringAssert.notNull(targetMap, "'targetMap' must not be null");
		if (targetMap instanceof SpringUnmodifiableMultiValueMap) {
			return (SpringMultiValueMap<K, V>) targetMap;
		}
		return new SpringUnmodifiableMultiValueMap<>(targetMap);
	}
	...
	public static <K, V> Map<K, V> compositeMap(Map<K,V> first, Map<K,V> second) {
		return new SpringCompositeMap<>(first, second);
	}
	...
	public static <K, V> Map<K, V> compositeMap(Map<K,V> first, Map<K,V> second,
		...
		return new SpringCompositeMap<>(first, second, putFunction, putAllFunction);
	}
