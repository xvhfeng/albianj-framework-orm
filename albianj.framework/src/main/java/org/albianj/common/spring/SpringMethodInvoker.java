...
public class SpringMethodInvoker {
	...
	public void prepare() throws ClassNotFoundException, NoSuchMethodException {
		...
		SpringAssert.notNull(targetClass, "Either 'targetClass' or 'targetObject' is required");
		SpringAssert.notNull(targetMethod, "Property 'targetMethod' is required");
	...
	protected Class<?> resolveClassName(String className) throws ClassNotFoundException {
		return SpringClassUtils.forName(className, SpringClassUtils.getDefaultClassLoader());
	}
	...
	protected Method findMatchingMethod() {
		...
		Class<?> targetClass = getTargetClass();
		SpringAssert.state(targetClass != null, "No target class set");
		Method[] candidates = SpringReflectionUtils.getAllDeclaredMethods(targetClass);
	...
	public Object invoke() throws InvocationTargetException, IllegalAccessException {
		...
		SpringReflectionUtils.makeAccessible(preparedMethod);
	...
	public static int getTypeDifferenceWeight(Class<?>[] paramTypes, Object[] args) {
		...
		for (int i = 0; i < paramTypes.length; i++) {
			if (!SpringClassUtils.isAssignableValue(paramTypes[i], args[i])) {
			...
			if (args[i] != null) {
				...
				while (superClass != null) {
					...
					else if (SpringClassUtils.isAssignable(paramType, superClass)) {
						result = result + 2;
