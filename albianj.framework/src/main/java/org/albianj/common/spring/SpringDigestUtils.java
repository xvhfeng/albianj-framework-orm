 ...
 * @since 3.0
 */
public abstract class SpringDigestUtils {
	...
	private static byte[] digest(String algorithm, InputStream inputStream) throws IOException {
		...
		if (inputStream instanceof SpringUpdateMessageDigestInputStream digestInputStream){
			digestInputStream.updateMessageDigest(messageDigest);
			return messageDigest.digest();
		}
		else {
			final byte[] buffer = new byte[SpringStreamUtils.BUFFER_SIZE];
			int bytesRead;
